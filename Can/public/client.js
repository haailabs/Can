  ...
  if (isConnected) {
    ...
    if (yourTasksElement.style.display === "block") {
      ...
      for (var i = yourNumHIPS - 1; i >= 0; i--) {
        ...
        exportButton.textContent = 'Download responses';
        exportButton.classList.add('uk-button', 'uk-button-default', 'uk-button-small', 'uk-margin-small-top', 'fixed-width-button');

        // Publish button 
        let publishButton = document.createElement('button');
        publishButton.textContent = 'Publish Dataset';
        publishButton.classList.add('uk-button', 'uk-button-default', 'uk-button-small', 'uk-margin-small-top', 'fixed-width-button');
        ...
        if (taskType == "choice") {
          ...
          publishButton.style.background = "var(--primary-1)";
          ...
          publishButton.style.color = "white";
          publishButton.style.background = "var(--primary-2)";
        }
        if (taskType == "sorting") {
          icon.setAttribute('data-eva', 'keypad-outline');
          li.style.setProperty("--color", "var(--primary-3)");
          ...
          publishButton.style.color = "white";
          publishButton.style.background = "var(--primary-3)";
        }
        if (taskType == "labelling") {
          icon.setAttribute('data-eva', 'pricetags-outline');
          li.style.setProperty("--color", "var(--primary-4)");
          ...
          publishButton.style.color = "white";
          publishButton.style.background = "var(--primary-4)";
...
publishButton.addEventListener('click', () => {
  ...
  .then(data => {
    console.log(`DataNft address ${data.nftAddress}`);
    const chainId = 80001;
    const checksum = CryptoJS.SHA256(data.nftAddress + chainId.toString()).toString(CryptoJS.enc.Hex);
    const did = 'did:op:' + checksum;
    
    console.log(did);

    // Assuming setMetadata is now to be called
    return fetch('/setMetadata', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        ...
        // Include other required fields for setMetadata here
      })
    });
  })
  .then(response => response.json())
  .then(metadataResponse => {
    console.log('Metadata set successfully:', metadataResponse);
    // Handle success response
  })
  .catch((err) => {
    console.error("Error setting metadata or creating NFT:", err);
  });
});
        ...
        buttonContainer.style.display = "flex"; // Set to display content side by side
        buttonContainer.style.gap = "10px"; // Adds a 10px gap between items
        ...
        // Append the new container to the anchor
